...
import static se.jbee.task.api.Param.command;
import static se.jbee.task.api.Param.serial;
import static se.jbee.task.api.Param.task;
import static se.jbee.task.api.Param.version;
import static se.jbee.task.api.Param.viewed;
...
public final class Params extends EnumMap<Param, String> {
	 ...
	 *  /{output}/{area}/{serial}
	 * </pre>
	 * All POST/PUT URLs set <code>modification</code> to <code>true</code>.
	 */
	public static Params fromPath(String path, boolean modification) {
		if (path.startsWith("/"))
		...
		if (!path.isEmpty()) {
			...
			if (modification || "do".equalsIgnoreCase(s0)) { // just for now also check for /do/ for ease of testing
				//TODO just for now
				...
				.set(Param.task, "10");
			} else if ("user".equals(s0) || "*".equals(s0)) {
