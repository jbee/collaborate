...
import static se.jbee.task.engine.Change.Operation.abandon;
import static se.jbee.task.engine.Change.Operation.absolve;
import static se.jbee.task.engine.Change.Operation.advance;
import static se.jbee.task.engine.Change.Operation.archive;
import static se.jbee.task.engine.Change.Operation.aspire;
import static se.jbee.task.engine.Change.Operation.attach;
import static se.jbee.task.engine.Change.Operation.authenticate;
import static se.jbee.task.engine.Change.Operation.categorise;
import static se.jbee.task.engine.Change.Operation.compart;
import static se.jbee.task.engine.Change.Operation.compose;
import static se.jbee.task.engine.Change.Operation.configure;
import static se.jbee.task.engine.Change.Operation.confirm;
import static se.jbee.task.engine.Change.Operation.connect;
import static se.jbee.task.engine.Change.Operation.consent;
import static se.jbee.task.engine.Change.Operation.detach;
import static se.jbee.task.engine.Change.Operation.disclose;
import static se.jbee.task.engine.Change.Operation.disconnect;
import static se.jbee.task.engine.Change.Operation.dissent;
import static se.jbee.task.engine.Change.Operation.dissolve;
import static se.jbee.task.engine.Change.Operation.emphasise;
import static se.jbee.task.engine.Change.Operation.envision;
import static se.jbee.task.engine.Change.Operation.erase;
import static se.jbee.task.engine.Change.Operation.indicate;
import static se.jbee.task.engine.Change.Operation.leave;
import static se.jbee.task.engine.Change.Operation.open;
import static se.jbee.task.engine.Change.Operation.participate;
import static se.jbee.task.engine.Change.Operation.poll;
import static se.jbee.task.engine.Change.Operation.propose;
import static se.jbee.task.engine.Change.Operation.rebase;
import static se.jbee.task.engine.Change.Operation.recompose;
import static se.jbee.task.engine.Change.Operation.register;
import static se.jbee.task.engine.Change.Operation.release;
import static se.jbee.task.engine.Change.Operation.relocate;
import static se.jbee.task.engine.Change.Operation.remind;
import static se.jbee.task.engine.Change.Operation.rephrase;
import static se.jbee.task.engine.Change.Operation.request;
import static se.jbee.task.engine.Change.Operation.resolve;
import static se.jbee.task.engine.Change.Operation.suggest;
import static se.jbee.task.engine.Change.Operation.tag;
import static se.jbee.task.engine.Change.Operation.unwatch;
import static se.jbee.task.engine.Change.Operation.warn;
import static se.jbee.task.engine.Change.Operation.watch;
...
public interface Change {
	...
	public static Change open(Name output, Name board, Name actor, Cause cause, Goal goal) {
		return (t, tx) -> tx.put(open, t.open(tx.output(output), board, tx.user(actor), cause, goal));
	}
	...
	static Change indicate(Name output, Gist gist, Name reporter, Name area) {
		return (t, tx) -> tx.put(indicate, t.reportDirection(tx.output(output), gist, tx.user(reporter), tx.area(output, area)));
	}
	...
	public static Change advance(Name output, IDN basis, Cause cause, Goal goal, Gist gist, Name reporter) {
		return (t, tx) -> tx.put(advance, t.reportAdvancement(tx.task(output, basis), cause, goal, gist, tx.user(reporter)));
	}
