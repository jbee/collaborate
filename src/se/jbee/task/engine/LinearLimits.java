 ...
 * {@link #block(Limit, Clock)}ed. When allocated any further attempt to stress
 * or occupy will cause a {@link ConcurrentUsage} till the occupied
 * {@link Limit} is {@link #unblock(Limit)}ed again.
 *
...
public final class LinearLimits implements Limits {
	...
	public boolean block(Limit l, Clock clock) throws ConcurrentUsage {
		...
		if (!limits.blocked.compareAndSet(false, true)) {
	...
	public void unblock(Limit l) {
		if (!periodLimits(l, null).blocked.compareAndSet(true, false)) {
			throw new ConcurrentUsage(l);
		}
	}
